...
sealed trait Postnatalis[+T <: AggregateRoot] extends AggregateRootLifecycle[T]{ def version: AggregateRootVersion }
...
final case class Vivus[T <: AggregateRoot](ar: T) extends Postnatalis[T] with Antemortem[T]{ def version: AggregateRootVersion = ar.version }
