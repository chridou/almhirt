...
object WorklowTerminatators {
  ...
  def finishTerminalF[TFrom, T, U, TTo](toResponse: (TFrom) => AlmFuture[HttpResponse])(implicit respConsumer: ResponseConsumer[TTo], problemConsumer: Consumer[Problem], executionContext: ExecutionContext): (TFrom, TTo) => Unit =
    (source: TFrom, dest: TTo) =>
      toResponse(source).onComplete(
        fail => problemConsumer(fail),
        succ => respConsumer(dest, succ))

  def finishWithResponseResultF[TFrom, T, U, TRes](toResponse: (TFrom) => AlmFuture[HttpResponse])(implicit gen: SpecialResponseGenerator[TRes], executionContext: ExecutionContext): (TFrom) => AlmFuture[TRes] =
    (source: TFrom) =>
