...
trait AlmValidationCastFunctions {
  ...
  def almCast[To](what: Any)(implicit tag: ClassTag[To]): AlmValidation[To] = {
    ...
    try {
      ...
      case exn: ClassCastException => TypeCastProblem(s"I can not cast from ${what.getClass.getName()} to ${tag.runtimeClass.getName()}", cause = Some(CauseIsThrowable(exn))).failure
    }
  }
}