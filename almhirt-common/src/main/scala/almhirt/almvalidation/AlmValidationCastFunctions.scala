...
object CastHelper {
  /** Taken from scala.concurrent, unfortunately its a private value
   * 
   */
...
trait AlmValidationCastFunctions {
  ...
  def almCast[To](what: Any)(implicit tag: ClassTag[To]): AlmValidation[To] = {
    ...
    try {
      ...
      case exn: ClassCastException => TypeCastProblem(s"Could not cast from ${what.getClass.getName()} to ${tag.runtimeClass.getName()}", cause = Some(CauseIsThrowable(exn))).failure
    }
  }
}