...
import almhirt.validation.Problem.BadDataProblem

class ProblemSpecs extends Specification {
  val bdpA = BadDataProblem("Message A", "A", Minor)
  val bdpB = BadDataProblem("Message B", "B", Major)
  
  "A BadDataProblem with key = 'A' and message = 'Message A' when transformed to a MultipleBadDataProblem" should {
    ...
    "have the same severity as the original BadDataProblem" in {
      bdpA.toMultipleBadData.severity must beEqualTo(bdpA.severity)
    }
  }
  
  "A MultipleBadDataProblem created by BadDataProblems A and B with different severities and different keys" should {
  ...
  "A MultipleBadDataError having added an already contained key with a different message" should {
    ...
    "keep the severity when a MultipleBadDataObject with lower severity is added(combineWith)" in {
      (bdpA add bdpB).combineWith(BadDataProblem("Message C", "A", Minor).toMultipleBadData)
      .severity must beEqualTo((bdpA add bdpB).severity)
    }
    "keep the severity when a MultipleBadDataObject with same severity is added(combineWith)" in {
      (bdpA add bdpB).combineWith(BadDataProblem("Message C", "A", Major).toMultipleBadData)
      .severity must beEqualTo((bdpA add bdpB).severity)
    }
    "take the severity of the MultipleBadDataObject with higher severity is added(combineWith)" in {
      (bdpA add bdpB).combineWith(BadDataProblem("Message C", "A", Critical).toMultipleBadData)
      .severity must beEqualTo(Critical)
    }
