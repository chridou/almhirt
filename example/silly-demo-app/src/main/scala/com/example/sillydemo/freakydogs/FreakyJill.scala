...
class FreakyJill(implicit override val almhirtContext: AlmhirtContext) extends FreakyDogsActor {
  implicit val executionContext = almhirtContext.futuresContext

  private case object DoSomething
  private case object ChangeCircuitBreakerRegistered
  ...
  var circuitBreakerRegistered = false
  ...
  override def preStart() {
    ...
    self ! DoSomething
    self ! DoSomething
    cancel = context.system.scheduler.schedule(rnd.nextInt(4).seconds, (rnd.nextInt(6)+5).seconds, self, ChangeCircuitBreakerRegistered)
  }

  override def postStop() {
    cancel.cancel()
    if (circuitBreakerRegistered) deregisterCircuitControl()
  }
}
