...
class UnitOfWorkHandlerSpecs extends Specification {
  """A NewTestPersonUnitOfWork's handler""" should {
    """be able to create a new TestPerson when a valid name is supplied""" in {
      val res = NewTestPersonUnitOfWork.handler(NewTestPerson(java.util.UUID.randomUUID, "Harry"))
      res.isSuccess
    }
    """not be able to create a new TestPerson when an invalid name is supplied""" in {
      val res = NewTestPersonUnitOfWork.handler(NewTestPerson(java.util.UUID.randomUUID,""))
      res.isFailure
    }
    ...
    """create exactly one Event event""" in {
      val res = NewTestPersonUnitOfWork.handler(NewTestPerson(java.util.UUID.randomUUID,"Harry")).forceResult
      res._2.length === 1
    }
  ...
  """A ChangeTestPersonNameUnitOfWork's handler""" should {
    """return a success when a valid name is supplied incuding the correct version""" in {
      val originalEntity = NewTestPersonUnitOfWork.handler(NewTestPerson(java.util.UUID.randomUUID,"Harry")).forceResult._1
