...
case class DomainEventLogReadStatistics(
  ...
  def add(ms: Long) = {
    ...
    DomainEventLogReadStatistics(newMin, newMax, newAvg, newTotal, newCount, newCount0Ms)
  }

  override def toString() = {
    s"""DomainEventLogReadStatistics(minMs=$minMs, maxMs=$maxMs, avgMs=$avgMs, totalMs=$totalMs, count=$count, count0Ms=$count0Ms)"""
  }
}

object DomainEventLogReadStatistics {
  def apply(): DomainEventLogReadStatistics =
    DomainEventLogReadStatistics(0L, 0L, 0L, 0L, 0L, 0L)
}