...
object ActorMessages {
  final case class CreateChildActor(factory: ComponentFactory, name: Option[String], returnActorRef: Boolean)
...
object CreateChildActorHelper {
  ...
  def createChildActor(parent: ActorRef, factory: ComponentFactory, name: Option[String])(maxDur: FiniteDuration)(implicit execCtx: ExecutionContext): AlmFuture[ActorRef] = {
    parent.ask(ActorMessages.CreateChildActor(factory, name, true))(maxDur).successfulAlmFuture[ActorMessages.CreateChildActorRsp].mapV {
