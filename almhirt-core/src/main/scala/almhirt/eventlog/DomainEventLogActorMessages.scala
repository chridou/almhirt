...
case class LoggedDomainEventsRsp(committedEvents: IndexedSeq[DomainEvent], uncommittedEvents: IndexedSeq[(Problem,DomainEvent)], correlationId: Option[UUID]) extends DomainEventLogRsp {
