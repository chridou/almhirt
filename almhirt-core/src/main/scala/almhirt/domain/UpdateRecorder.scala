 ...
 * Writer monad.
 */
trait UpdateRecorder[+Event <: EntityEvent, +ES <: Entity[_, _]] {
   ...
   */
  def map[EES <: Entity[_, _]](f: ES => EES): UpdateRecorder[Event, EES] =
  	UpdateRecorder[Event, EES] { events =>
   ...
   */
  def flatMap[EEvent >: Event <: EntityEvent , EES <: Entity[_, _]](f: ES => UpdateRecorder[EEvent, EES]): UpdateRecorder[EEvent, EES] =
  	UpdateRecorder[EEvent, EES] { events =>
  	  val (currentEvents, validation) = this(events)
  	  validation fold (
  	    problem => 
...
object UpdateRecorder {
  ...
  def startWith[Event <: EntityEvent, ES <: Entity[_, _]](entity: ES) =
    UpdateRecorder[Event, ES](events => (events, entity.success))
  ...
  def accept[Event <: EntityEvent, ES <: Entity[_, _]](event: Event, entity: ES) =
    UpdateRecorder[Event, ES](events => (event :: events, entity.success))
  ...
  def reject[Event <: EntityEvent, ES <: Entity[_, _]](error: Problem) =
    UpdateRecorder[Event, ES](events => (events, error.failure))
