...
abstract class CommandExecutorFullDownstreamSpecsTemplate(theActorSystem: ActorSystem)
  ...
  describe("CommandExecutor with independent commands") {
  ...
  describe("CommandExecutor with a sequence") {
    it("should emit ExecutionStarted(1) -> ExecutionInProcess(2) -> ExecutionSuccessful(3) when tracking is enabled on the sequence") {
      useExecutorWithEventLog {
        (executor, eventLog, probe) ⇒
          val aggRef = AggregateRootRef(theAlmhirt.getUuid)
          val groupId = "track me"
          val theCommands =
            List[AR1Command](
              AR1ComCreateAR1(DomainCommandHeader(aggRef), "a"),
              AR1ComChangeB(DomainCommandHeader(aggRef), Some("B")),
              AR1ComChangeA(DomainCommandHeader(aggRef), "s"))
          val groupedCommands = CommandGrouping.groupCommands(groupId, theCommands, trackableAsGroup = true)
          groupedCommands.foreach(executor ! _)
          val res = probe.receiveWhile(defaultDuration, defaultDuration, 9) {
            case Message(_, ExecutionStateChanged(_, m: ExecutionStarted)) if (m.trackId == groupId) ⇒ Some(1)
            case Message(_, ExecutionStateChanged(_, m: ExecutionInProcess)) if (m.trackId == groupId) ⇒ Some(2)
            case Message(_, ExecutionStateChanged(_, m: ExecutionSuccessful)) if (m.trackId == groupId) ⇒ Some(3)
            case Message(_, m: CommandExecuted) ⇒ None
            case Message(_, m: CommandReceived) ⇒ None
          }.flatten
          res should equal(1 :: 2 :: 3 :: Nil)
    ...
    it("should emit ExecutionStarted(1) -> ExecutionInProcess(2) -> ExecutionFailed(3) when tracking is enabled on the sequence but one command of the sequence sets an invalid value") {
      useExecutorWithEventLog {
        (executor, eventLog, probe) ⇒
          val aggRef = AggregateRootRef(theAlmhirt.getUuid)
          val groupId = "track me"
          ...
          val res = probe.receiveWhile(defaultDuration, defaultDuration, 9) {
            case Message(_, ExecutionStateChanged(_, m: ExecutionStarted)) if (m.trackId == groupId) ⇒ Some(1)
            case Message(_, ExecutionStateChanged(_, m: ExecutionInProcess)) if (m.trackId == groupId) ⇒ Some(2)
            case Message(_, ExecutionStateChanged(_, m: ExecutionFailed)) if (m.trackId == groupId) ⇒ Some(3)
            case Message(_, m: CommandExecuted) ⇒ None
            case Message(_, m: CommandNotExecuted) ⇒ None
            case Message(_, m: CommandReceived) ⇒ None
          }.flatten
    ...
    it("should emit ExecutionStarted(1) -> ExecutionSuccessful(3) for each command when tracking is not enabled on the sequence but on each command") {
      useExecutorWithEventLog {
        (executor, eventLog, probe) ⇒
          val aggRef = AggregateRootRef(theAlmhirt.getUuid)
          val groupId = "track me grp"
          val trackId1 = "track me 1"
          val trackId2 = "track me 2"
          val trackId3 = "track me 3"
          val theCommands =
            List[AR1Command](
              AR1ComCreateAR1(DomainCommandHeader(aggRef), "a").track(trackId1),
              AR1ComChangeB(DomainCommandHeader(aggRef), Some("B")).track(trackId2),
              AR1ComChangeA(DomainCommandHeader(aggRef), "s").track(trackId3))
          val groupedCommands = CommandGrouping.groupCommands(groupId, theCommands, trackableAsGroup = true)
          groupedCommands.foreach(executor ! _)
          val res = probe.receiveWhile(defaultDuration, defaultDuration, 18) {
            case Message(_, ExecutionStateChanged(_, m: ExecutionStarted)) if (m.trackId == groupId) ⇒ Some(1)
            case Message(_, ExecutionStateChanged(_, m: ExecutionInProcess)) if (m.trackId == groupId) ⇒ Some(2)
            case Message(_, ExecutionStateChanged(_, m: ExecutionStarted)) if (m.trackId == trackId1) ⇒ Some(3)
            case Message(_, ExecutionStateChanged(_, m: ExecutionSuccessful)) if (m.trackId == trackId1) ⇒ Some(4)
            case Message(_, ExecutionStateChanged(_, m: ExecutionStarted)) if (m.trackId == trackId2) ⇒ Some(5)
            case Message(_, ExecutionStateChanged(_, m: ExecutionSuccessful)) if (m.trackId == trackId2) ⇒ Some(6)
            case Message(_, ExecutionStateChanged(_, m: ExecutionStarted)) if (m.trackId == trackId3) ⇒ Some(7)
            case Message(_, ExecutionStateChanged(_, m: ExecutionSuccessful)) if (m.trackId == trackId3) ⇒ Some(8)
            case Message(_, ExecutionStateChanged(_, m: ExecutionSuccessful)) if (m.trackId == groupId) ⇒ Some(9)
            case Message(_, m: CommandExecuted) ⇒ None
            case Message(_, m: CommandReceived) ⇒ None
          }.flatten
          res should equal((1 to 9).toList)
      }
    }

    it("should emit ExecutionStarted -> ExecutionSuccessful for each command and ExecutionStarted -> ExecutionInProcess -> ExecutionSuccessful for the sequence when tracking is enabled on the sequence and on each command") {
      useExecutorWithEventLog {
        (executor, eventLog, probe) ⇒
          val aggRef = AggregateRootRef(theAlmhirt.getUuid)
          val trackId1 = "track me 1"
          ...
          groupedCommands.foreach(executor ! _)
          val res = probe.receiveWhile(defaultDuration, defaultDuration, 18) {
            case Message(_, ExecutionStateChanged(_, m: ExecutionStarted)) if (m.trackId == trackId1) ⇒ Some(1)
            case Message(_, ExecutionStateChanged(_, m: ExecutionSuccessful)) if (m.trackId == trackId1) ⇒ Some(2)
            case Message(_, ExecutionStateChanged(_, m: ExecutionStarted)) if (m.trackId == trackId2) ⇒ Some(3)
            case Message(_, ExecutionStateChanged(_, m: ExecutionSuccessful)) if (m.trackId == trackId2) ⇒ Some(4)
            case Message(_, ExecutionStateChanged(_, m: ExecutionStarted)) if (m.trackId == trackId3) ⇒ Some(5)
            case Message(_, ExecutionStateChanged(_, m: ExecutionSuccessful)) if (m.trackId == trackId3) ⇒ Some(6)
            case Message(_, m: CommandExecuted) ⇒ None
            case Message(_, m: CommandReceived) ⇒ None
          }.flatten
          res should equal((1 to 6).toList)
      }
    }
    ...
    //              AR1ComChangeB(DomainCommandHeader(aggRef), Some("")),
    //              AR1ComChangeA(DomainCommandHeader(aggRef), "s"))
    //          val groupedCommands = CommandGrouping.groupCommands("mygroup", theCommands, trackableAsGroup = true)
    //          groupedCommands.foreach(executor ! _)
    //          val res = probe.receiveWhile(defaultDuration, defaultDuration, 9) {
    //            case Message(_, ExecutionStateChanged(_, m: ExecutionStarted)) if (m.trackId == trackId) ⇒ Some(1)
    //            case Message(_, ExecutionStateChanged(_, m: ExecutionInProcess)) if (m.trackId == trackId) ⇒ Some(2)
