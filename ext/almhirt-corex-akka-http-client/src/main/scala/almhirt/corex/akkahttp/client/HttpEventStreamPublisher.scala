...
object HttpEventStreamPublisher {
  def propsRaw(
    ...
    missedEventSeverity: almhirt.problem.Severity)(implicit serializer: HttpSerializer[Event], problemDeserializer: HttpDeserializer[Problem], almhirtContext: AlmhirtContext, materializer: Materializer): Props =
  ...
  def props(httpEventPublisherName: String)(implicit ctx: AlmhirtContext, serializer: HttpSerializer[Event], problemDeserializer: HttpDeserializer[Problem], materializer: Materializer): AlmValidation[Props] = {
    implicit val extr = almhirt.httpx.akkahttp.HttpMethodConfigExtractor
    ...
    for {
      ...
      res ← if (enabled) {
        for {
          ...
          mediaType ← inTryCatch { MediaType.custom(contentMediaTypeStr, binary = false, MediaType.Compressible, List.empty) }
