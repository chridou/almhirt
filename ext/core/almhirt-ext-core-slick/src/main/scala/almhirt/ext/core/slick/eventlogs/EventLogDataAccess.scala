...
class TextEventLogDataAccess(override val eventlogTablename: String, override val blobTablename: String, override val getDb: Unit => Database, override val profile: scala.slick.driver.ExtendedProfile, override val hasExecutionContext: HasExecutionContext) extends BlobStoreComponent with TextEventLogStoreComponent with Profile {
  ...
  def storeRowAndBlobs(row: TextEventLogRow, blobs: Vector[(JUUID, Array[Byte])]): AlmValidation[TextEventLogRow] = {
    computeSafely {
...
class BinaryLogDataAccess(override val eventlogTablename: String, override val blobTablename: String, override val getDb: Unit => Database, override val profile: scala.slick.driver.ExtendedProfile, override val hasExecutionContext: HasExecutionContext) extends BlobStoreComponent with BinaryEventLogStoreComponent with Profile {
  import profile.simple._
  ...
  def storeRowAndBlobs(row: BinaryEventLogRow, blobs: Vector[(JUUID, Array[Byte])]): AlmValidation[BinaryEventLogRow] = {
    computeSafely {
      getDb() withTransaction { implicit session: Session =>
