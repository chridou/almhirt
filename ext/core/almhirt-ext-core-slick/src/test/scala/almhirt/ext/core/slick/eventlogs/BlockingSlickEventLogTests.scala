...
//  }
  ...
  test("The eventlog must store the events via the messagehub and then return all events") {
    (inLocalTestAlmhirt { almhirt => 
      //almhirt.getService[DomainEventsChannel].foreach(channel => channel.<-<#[DomainEvent]((ev: Event) => println(ev)))
      for {
        eventlog <- almhirt.getService[EventLog]
        _ <- inTryCatch { events.foreach(event => almhirt.publish(Message(event))); Thread.sleep(50) }
