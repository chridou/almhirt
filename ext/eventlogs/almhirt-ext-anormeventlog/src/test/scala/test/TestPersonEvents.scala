...
class TestPersonCreatedDecomposer extends Decomposer[TestPersonCreated] {
  ...
  def decompose[TChannel <: RiftChannel, TDimension <: RiftTypedDimension[_]](what: TestPersonCreated)(implicit into: Dematerializer[TChannel, TDimension]): AlmValidation[Dematerializer[TChannel, TDimension]] = {
...
class TestPersonNameChangedDecomposer extends Decomposer[TestPersonNameChanged] {
  ...
  def decompose[TChannel <: RiftChannel, TDimension <: RiftTypedDimension[_]](what: TestPersonNameChanged)(implicit into: Dematerializer[TChannel, TDimension]): AlmValidation[Dematerializer[TChannel, TDimension]] = {
    into.addTypeDescriptor(typeDescriptor)
      .bind(_.addUuid("id", what.id))
      .bind(_.addUuid("aggId", what.aggId))
...
class TestPersonAddressAquiredDecomposer extends Decomposer[TestPersonAddressAquired] {
  ...
  def decompose[TChannel <: RiftChannel, TDimension <: RiftTypedDimension[_]](what: TestPersonAddressAquired)(implicit into: Dematerializer[TChannel, TDimension]): AlmValidation[Dematerializer[TChannel, TDimension]] = {
    into.addTypeDescriptor(typeDescriptor)
      .bind(_.addUuid("id", what.id))
      .bind(_.addUuid("aggId", what.aggId))
      .bind(_.addLong("aggVersion", what.aggVersion))
...
class TestPersonMovedDecomposer extends Decomposer[TestPersonMoved] {
  ...
  def decompose[TChannel <: RiftChannel, TDimension <: RiftTypedDimension[_]](what: TestPersonMoved)(implicit into: Dematerializer[TChannel, TDimension]): AlmValidation[Dematerializer[TChannel, TDimension]] = {
    into.addTypeDescriptor(typeDescriptor)
      .bind(_.addUuid("id", what.id))
      .bind(_.addUuid("aggId", what.aggId))
      .bind(_.addLong("aggVersion", what.aggVersion))
...
class TestPersonUnhandledEventDecomposer extends Decomposer[TestPersonUnhandledEvent] {
  ...
  def decompose[TChannel <: RiftChannel, TDimension <: RiftTypedDimension[_]](what: TestPersonUnhandledEvent)(implicit into: Dematerializer[TChannel, TDimension]): AlmValidation[Dematerializer[TChannel, TDimension]] = {
    into.addTypeDescriptor(typeDescriptor)
      .bind(_.addUuid("id", what.id))
      .bind(_.addUuid("aggId", what.aggId))
      .bind(_.addLong("aggVersion", what.aggVersion))
