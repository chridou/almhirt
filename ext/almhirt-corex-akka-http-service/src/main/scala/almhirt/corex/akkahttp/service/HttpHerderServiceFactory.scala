...
import almhirt.almfuture.all.FromFutureToAlmFutureOps0
import almhirt.common._
import almhirt.context.AlmhirtContext
import almhirt.corex.akkahttp.service.HerderServiceApp.HttpBind
...
object HttpHerderServiceFactory {
  ...
  def paramsFactory(implicit ctx: AlmhirtContext): AlmValidation[(ToEntityMarshaller[Problem]) ⇒ HttpHerderServiceParams] = {
    ...
    for {
      ...
      (problemMarshaller: ToEntityMarshaller[Problem]) ⇒ HttpHerderServiceParams(maxCallDuration, selector, problemMarshaller)
    }
  }
...
object HttpHerderService {
  def propsRaw(
    maxCallDuration: scala.concurrent.duration.FiniteDuration,
    exectionContextSelector: ExtendedExecutionContextSelector,
    problemMarshaller: ToEntityMarshaller[Problem])(implicit ctx: AlmhirtContext): Props =
  ...
  def props(problemMarshaller: ToEntityMarshaller[Problem])(implicit ctx: AlmhirtContext): AlmValidation[Props] =
    HttpHerderServiceFactory.paramsFactory.map(f ⇒ propsRaw(f(problemMarshaller)))

  def componentFactory(problemMarshaller: ToEntityMarshaller[Problem])(implicit ctx: AlmhirtContext): AlmValidation[ComponentFactory] =
...
trait HttpHerderServiceFactory extends Directives with ScalaXmlSupport { me: AlmActor with AlmActorLogging ⇒

  def createHerderServiceEndpoint(params: HttpHerderServiceFactory.HttpHerderServiceParams) = {
        ...
        pathEnd {
          get { ctx ⇒
            ...
            futHtml.fold(
              ...
              html ⇒ ctx.complete(StatusCodes.OK, html)).flatMap(f => f)
          }
        } ~ pathPrefix("circuits") {
          pathEnd {
          ...
          pathEnd {
            get { ctx ⇒
              ...
              fut.fold(
                problem ⇒ implicitly[AlmHttpProblemTerminator].terminateProblem(ctx, problem),
                reporters ⇒
                  ctx.complete(createStatusReporters(reporters, "../../../../herder"))).flatMap(f => f)
            }
          } ~ reportTerminator
          ...
          pathEnd {
            get { ctx ⇒
              ...
              fut.fold(
                problem ⇒ implicitly[AlmHttpProblemTerminator].terminateProblem(ctx, problem),
                entries ⇒ ctx.complete(StatusCodes.OK, createFailuresReport(entries, "../herder"))).flatMap(f => f)
            }
          ...
          pathEnd {
            get { ctx ⇒
              ...
              fut.fold(
                problem ⇒ implicitly[AlmHttpProblemTerminator].terminateProblem(ctx, problem),
                rejected ⇒ ctx.complete(StatusCodes.OK, createRejectedCommandsReport(rejected, "../herder"))).flatMap(f => f)
            }
          } ~ pathPrefix(Segment / Segment / IntNumber) { (appName, componentName, num) ⇒
            pathEnd {
              get { ctx ⇒
          ...
          pathEnd {
            get { ctx ⇒
              ...
              fut.fold(
                problem ⇒ implicitly[AlmHttpProblemTerminator].terminateProblem(ctx, problem),
                missed ⇒ ctx.complete(StatusCodes.OK, createMissedEventsReport(missed, "../herder"))).flatMap(f => f)
            }
          } ~ pathPrefix(Segment / Segment / IntNumber) { (appName, componentName, num) ⇒
            pathEnd {
              get { ctx ⇒
          ...
          pathEnd {
            get { ctx ⇒
              ...
              fut.fold(
                problem ⇒ implicitly[AlmHttpProblemTerminator].terminateProblem(ctx, problem),
                information ⇒ ctx.complete(StatusCodes.OK, createInformationReport(information, "../herder"))).flatMap(f => f)
            }
          } ~ pathPrefix(Segment / Segment / IntNumber) { (appName, componentName, num) ⇒
            pathEnd {
              get { ctx ⇒
