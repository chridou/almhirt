...
object HttpAggregateRootEventLogQueryEndpointFactory {
  final case class HttpAggregateRootEventLogQueryEndpointParams(
    ...
    eventMarshaller: ToEntityMarshaller[AggregateRootEvent],
    eventsMarshaller: ToEntityMarshaller[Seq[AggregateRootEvent]],
    problemMarshaller: ToEntityMarshaller[Problem])

  def paramsFactory(implicit ctx: AlmhirtContext): AlmValidation[(ActorRef, ToEntityMarshaller[AggregateRootEvent], ToEntityMarshaller[Seq[AggregateRootEvent]], ToEntityMarshaller[Problem]) ⇒ HttpAggregateRootEventLogQueryEndpointParams] = {
    ...
    for {
      ...
      (aggragateRootEeventLog: ActorRef, aggregateRootEventMarshaller: ToEntityMarshaller[AggregateRootEvent], aggregateRootEventsMarshaller: ToEntityMarshaller[Seq[AggregateRootEvent]], problemMarshaller: ToEntityMarshaller[Problem]) ⇒
...
trait HttpAggregateRootEventLogQueryEndpointFactory extends Directives { me: Actor with AlmHttpEndpoint with HasAlmhirtContext ⇒
  import almhirt.eventlog.AggregateRootEventLog

  def createAggregateRootEventLogQueryEndpoint(params: HttpAggregateRootEventLogQueryEndpointFactory.HttpAggregateRootEventLogQueryEndpointParams) = {
    ...
    pathPrefix("aggregate-root-event-log") {
      pathEnd {
        get {
            ...
            implicit ctx ⇒
              val fut =
                for {
                  ...
                  rsp ← (params.aggregateRootEventLog ? eventLogMessage)(params.maxQueryDuration).mapCastTo[AggregateRootEventLog.GetManyAggregateRootEventsResponse]
                  events ← rsp match {
                ...
                implicit ctx ⇒
                  val fut =
                    for {
                      ...
                      res ← (params.aggregateRootEventLog ? AggregateRootEventLog.GetAggregateRootEvent(validatedEventId))(params.maxQueryDuration)
