...
trait NoneIsHandledUnified[TDimension <: RiftDimension] { dematerializer: Dematerializer[TDimension] =>
  ...
  override def addOptionalPrimitiveMA[M[_], A](ident: String, ma: Option[M[A]])(implicit mM: ClassTag[M[_]], mA: ClassTag[A]) = option.cata(ma)(addPrimitiveMA(ident, _), noneHandler(ident).success)
  override def addOptionalComplexMA[M[_], A <: AnyRef](decomposer: Decomposer[A])(ident: String, ma: Option[M[A]])(implicit mM: ClassTag[M[_]], mA: ClassTag[A]) = option.cata(ma)(addComplexMA(decomposer)(ident, _), noneHandler(ident).success)
  override def addOptionalComplexMAFixed[M[_], A <: AnyRef](ident: String, ma: Option[M[A]])(implicit mM: ClassTag[M[_]], mA: ClassTag[A]) = option.cata(ma)(addComplexMAFixed(ident, _), noneHandler(ident).success)
  override def addOptionalComplexMALoose[M[_], A <: AnyRef](ident: String, ma: Option[M[A]])(implicit mM: ClassTag[M[_]], mA: ClassTag[A]) = option.cata(ma)(addComplexMALoose(ident, _), noneHandler(ident).success)
  override def addOptionalMA[M[_], A <: Any](ident: String, ma: Option[M[A]])(implicit mM: ClassTag[M[_]], mA: ClassTag[A]) = option.cata(ma)(addMA(ident, _), noneHandler(ident).success)
  ...
  override def addOptionalIterableWith[A <: AnyRef, Coll](ident: String, what: Option[IterableLike[A, Coll]], backupRiftDescriptor: Option[RiftDescriptor]): AlmValidation[Dematerializer[TDimension]] = option.cata(what)(addIterableWith(ident, _, backupRiftDescriptor), noneHandler(ident).success)
