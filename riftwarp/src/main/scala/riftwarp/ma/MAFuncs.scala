...
object MAFuncs {
  def map[M[_], A, B](ma: M[A])(map: (A) => B)(implicit functions: HasFunctionObjects, mM: ClassTag[M[_]]): AlmValidation[M[B]] = {
  ...
  def mapV[M[_], A, B](ma: M[A])(map: (A) => AlmValidation[B])(implicit functions: HasFunctionObjects, mM: ClassTag[M[_]]): AlmValidation[M[B]] = {
    option.cata(functions.tryGetMAFunctions[M])(
  ...
  def mapi[M[_], A, B](ma: M[A])(map: (A, String) => B)(implicit functions: HasFunctionObjects, mM: ClassTag[M[_]]): AlmValidation[M[B]] = {
    option.cata(functions.tryGetMAFunctions[M])(
      fo =>
        (fo: @unchecked) match {
...
//      fo => fo.sequenceValidations(fo.map(fo.map(ma)(map))(x => x.toAgg)),
  ...
  def mapiV[M[_], A, B](ma: M[A])(map: (A, String) => AlmValidation[B])(implicit functions: HasFunctionObjects, mM: ClassTag[M[_]]): AlmValidation[M[B]] = {
    option.cata(functions.tryGetMAFunctions[M])(
      fo =>
        (fo: @unchecked) match {
          case fo: LinearMAFunctions[M] =>
  ...
  def fold[M[_], A, B](channel: RiftChannel)(ma: M[A])(implicit functions: HasFunctionObjects, mM: ClassTag[M[_]], mA: ClassTag[A], mB: ClassTag[B]): AlmValidation[B] = {
    option.cata(functions.tryGetMAFunctions[M])(
