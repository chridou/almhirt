...
case object RiftHttpDataWithoutContent extends RiftHttpData { val contentType = RiftHttpNoContentContentType }
  ...
  implicit class RiftHttpDataOps1(data: RiftHttpData) {
    def toRiftDimension(): AlmValidation[RiftHttpDimension] =
      data match {
        ...
        case RiftHttpDataWithoutContent => OperationNotSupportedProblem("'toRiftDimension' not allowed for RiftHttpDataWithoutContent").failure
      }
    def getContentType(): RiftHttpContentType =
      data match {
        case RiftHttpDataWithContent(contentType, _) => contentType
        case RiftHttpDataWithoutContent => RiftHttpNoContentContentType
      }
  }
}
