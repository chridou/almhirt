...
class RiftWarpHttpFunsSpecs extends Specification {
  ...
  """createHttpDataFromRequest""" should {
    """return Success(RiftHttpDataWithoutContent) for a None content type and an empty body""" in {
      createHttpDataFromRequest(() => None, bodyType => RiftStringBody("").success)(settings.contentTypeOps) must beEqualTo(scalaz.Success(RiftHttpDataWithoutContent))
    }
    """return Success(RiftHttpDataWithoutContent) for a None content type and a non empty body""" in {
      createHttpDataFromRequest(() => None, bodyType => RiftStringBody("a").success)(settings.contentTypeOps) must beEqualTo(scalaz.Success(RiftHttpDataWithoutContent))
    }
   ...
   val testdataPrimJsonWithoutDescriptor =  
    ...
    "transformFromHttpData(targeting 'PrimitiveTypes' from JSON)" should {
      """be able to recreate PrimitiveTypes from proper request data with RiftHttpChannelContentType(RiftChannel.Json)""" in {
        ...
        recreated must beEqualTo(scalaz.Success(testdataPrim))
      } 
      """be able to recreate PrimitiveTypes from proper request data with RiftHttpQualifiedContentType(RiftDescriptor(classOf[PrimitiveTypes]), RiftChannel.Json)""" in {
        val recreated = 
          transformFromHttpData[PrimitiveTypes](riftWarp)(RiftHttpDataWithContent(
      ...
      """be able to recreate PrimitiveTypes from proper request data with RiftHttpQualifiedContentType(RiftDescriptor(classOf[PrimitiveTypes]), RiftChannel.Json) when the target type is a supertype of the target type""" in {
        val recreated = 
      ...
      """be able to recreate PrimitiveTypes from proper request data with RiftHttpChannelContentType(RiftChannel.Json) even if the content has no RiftDescriptor when the type parameter is set to the target type""" in {
        val recreated = 
      ...
      """be able to recreate PrimitiveTypes with wrong RiftDescriptor in RiftHttpQualifiedContentType(RiftDescriptor(classOf[String]), RiftChannel.Json) because the content has the correct RiftDescriptor which has priority""" in {
        val recreated = 
          transformFromHttpData[PrimitiveTypes](riftWarp)(RiftHttpDataWithContent(
