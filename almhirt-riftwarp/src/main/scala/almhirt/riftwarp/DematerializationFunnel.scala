...
import scalaz.std._
trait NoneHasNoEffectDematerializationFunnel[TChannel <: RiftChannelDescriptor, TDimension <: RiftTypedDimension[_]] { dematerializer: Dematerializer[TChannel, TDimension] =>
  ...
  def addOptionalPrimitiveMA[M[_], A](ident: String, ma: Option[M[A]], dematerialzeMA: M[A] => TDimension) = option.cata(ma)(addPrimitiveMA(ident, _, dematerialzeMA), dematerializer.success) 
